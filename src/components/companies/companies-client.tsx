...
export function CompaniesClient({ initialCompanies, initialTotal }: CompaniesClientProps) {
  ...
  // To skip the first fetch since we have initial data
  const isInitialRender = useRef(true)

  const fetchCompanies = useCallback(async () => {
    // Only set loading if it's not the very first load (which was SSR)
    ...
    try {
      ...
      if (debouncedSearch) {
        query.name = { $regex: debouncedSearch, $options: "i" }
      }
  ...
  useEffect(() => {
    ...
    fetchCompanies()
  }, [page, limit, debouncedSearch, fetchCompanies])
